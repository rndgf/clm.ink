...
const STAR_SVG =
  '<svg class="w-3 h-3 shrink-0" aria-hidden="true" viewBox="0 0 20 20" fill="currentColor"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>';

const STAR_SVG_LARGE =
...
function renderSliderSlide(item, index) {
  ...
  <blockquote class="w-full max-w-2xl rounded-xl border border-neutral-800 bg-neutral-950/60 px-4 py-4 sm:px-6 sm:py-5">
    <div class="flex gap-0.5 text-neutral-600 mb-2" aria-hidden="true">${STAR_SVG.repeat(5)}</div>
    <p class="text-sm text-neutral-200 leading-relaxed">« ${text} »</p>
    <cite class="mt-2 not-italic text-xs text-neutral-500">— ${author}</cite>
  </blockquote>
</div>`;
}
...
function renderGridCard(item) {
  const text = escapeHtml(item.text);
  const author = escapeHtml(item.author);
  return `<blockquote class="rounded-2xl border border-neutral-800 bg-neutral-950/60 px-6 py-6 sm:px-8 sm:py-8">
  <div class="flex gap-0.5 text-neutral-600 mb-4" aria-hidden="true">${STAR_SVG_LARGE.repeat(5)}</div>
  <p class="text-sm sm:text-base text-neutral-200 leading-relaxed">« ${text} »</p>
  <cite class="mt-4 not-italic text-xs text-neutral-500">— ${author}</cite>
</blockquote>`;
}
...
export function runTestimonialsShuffle() {
  ...
  if (grid) {
    grid.innerHTML = shuffled.map((item) => renderGridCard(item)).join('');
  }
}
